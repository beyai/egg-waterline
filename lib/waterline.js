...
module.exports = app => {
  ...
  for (const key of Object.keys(config.db)) {
    switch (key) {
      case 'mongodb':
        wlconfig.adapters[key] = require('sails-mongo');
        break;
      case 'mysql':
        wlconfig.adapters[key] = require('sails-mysql');
        break;
      default :
        assert(key, 'adapter illegal');
        break;
    }
  }
  ...
  app.beforeStart(async () => {
    waterline.initialize(config, async (err, Models) => {
      if (err) {
        assert(err);
        return;
      }
